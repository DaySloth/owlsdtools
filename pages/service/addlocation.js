...
export default function AddServiceLocation({ issues }) {
  ...
  async function handleSubmit() {
    setLoading(true);
    const { data } = await (
      await fetch("http://localhost:3000/api/database/service/add", {
        method: "POST",
  ...
  async function searchLocation(location) {
    const { data } = await (
      await fetch(
        `http://localhost:3000/api/database/accounts/search/${location}`
      )
    ).json();
    const options = [];
    data.map((e) => {
  ...
  return (
    <main className={`${styles.servicePage} ${styles.owlbackground}`}>
      ...
      <Segment placeholder>
        <Header icon>Add Location</Header>
        <Card.Group>
          <Button
            inverted
            fluid
          ...
          >
            <Form.Field>
              ...
              <Input
                ...
                onChange={(e, d) => {
                  setLocationLookup(e.target.value);
                }}
                icon={
                  <Icon
                    name="search"
            ...
            <Form.Field>
              ...
              <Dropdown
                ...
                // Use this function to update the entry
                onChange={(e, data) => {
                  setNewEntry({ ...newEntry, Status: data.value });
                }}
              />
            </Form.Field>
